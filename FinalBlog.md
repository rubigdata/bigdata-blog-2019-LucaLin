...
For this assignment, we had to write code in scala which could be submitted to a cluster. This was much different from the notebooks, where I could write code and test it immediately. However, the standalone spark program gave me a lot of issues which I will outline in this section. 

At first, I tried to copy the code from the notebooks to the standalone program. However, very quickly I received error messages telling me that types were not valid due to serialization and type errors. The Jsoup classes also had to be imported inside the build.sbt file, which i realized after getting an error telling me that Jsoup could not be imported and quickly googling the solution. I have also had a lot of trouble with sequences, lists and  the reduceByKey operation. The network kept denying the job, but when I sampled only a part of the WARC file, it worked and gave a result. I speculate that memory was the issue, which is my next point. 
